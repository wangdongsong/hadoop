...
public class CartesianMRJobRunner extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        JobConf jobConf = new JobConf("CartesianMRJobRunner");
        jobConf.setJarByClass(CartesianMRJobRunner.class);
        jobConf.setMapperClass(CartesianMapper.class);
        jobConf.setNumMapTasks(0);
        jobConf.setInputFormat(CartesianInputFormat.class);

        CartesianInputFormat.setLeftInputInfo(jobConf, TextInputFormat.class, args[0]);
        CartesianInputFormat.setRightInputInfo(jobConf, TextInputFormat.class, args[0]);
    ...
    private class CartesianMapper implements Mapper {
        ...
        public void map(Object key, Object value, OutputCollector output, Reporter reporter) throws IOException {
            if (!key.toString().equalsIgnoreCase(value.toString())) {
                ...
                String[] rightTokens = key.toString().split("\\s");
                Set<String> leftSet = new HashSet<>(Arrays.asList(leftTokens));
                Set<String> rigthSet = new HashSet<>(Arrays.asList(rightTokens));

                StringBuilder words = new StringBuilder();
                List wordList =  leftSet.stream().filter(s ->rigthSet.contains(s)).collect(Collectors.toList());
                if (wordList != null & wordList.size() > 2) {
                    wordList.forEach(str -> words.append(str + ","));
    ...
    private static class CartesianInputFormat extends FileInputFormat {
